    ...
    if (isModalVisible && layers.length === 0) {
      ...
      feature.setStyle(
        new Style({
          ...
          fill: new Fill({ color: [255, 27, 27, 0.2] }),
        }),
      );
      ...
      const vectorLayer = new VectorLayer({
        ...
        properties: {
          name: (
            <Popover content={JSON.stringify(geom)} title="geojson">
              {_geomName} <InfoCircleOutlined />
            </Popover>
          ),
          id: uuid,
        },
    ...
    <>
      <Button onClick={showModal}>
        {buttonText ? (
          ...
          <Tooltip title={intl.formatMessage({ id: 'component.modalMap.buttonTooltipTitle' })}>
