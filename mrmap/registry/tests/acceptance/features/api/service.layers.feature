Feature: Layers
  ...
  Scenario: Accessing the services API endpoint without any search criteria will return all services
    ...
    And all layers objects are returned
  ...
  Scenario: Only resources which has keyword, title, abstract or uuid like "WMS" is part of the response
    Given an authorized session
    Given a request url http://localhost:8000/api/v1/registry/service/layers/?search=WMS&ordering=title
    When the request sends GET
    Then the response status is OK
    And 137 result(s) is(are) returned, with WMS as search criteria
    And a paginated response is returned
    And response objects are ordered ascending by title
    And "accessible" key is part of the response
  ...
  Scenario: Only resources in the given date range are part of the response.
    Given an authorized session
    Given a request url http://localhost:8000/api/v1/registry/service/layers/?registration_min=2021-10-12&registration_max=2021-10-20&ordering=title
    When the request sends GET
    Then the response status is OK
    And 137 result(s) is(are) returned, between 2021-10-12 and 2021-10-20 as registration range criteria
    And a paginated response is returned
    And response objects are ordered ascending by title
    And "accessible" key is part of the response
  ...
  Scenario: Only resources in the given datestamp range are part of the response.
    Given an authorized session
    Given a request url http://localhost:8000/api/v1/registry/service/layers/?datestamp_min=2021-10-20&datestamp_max=2021-10-23&ordering=title
    When the request sends GET
    Then the response status is OK
    And 137 result(s) is(are) returned, between 2021-10-20 and 2021-10-23 as datestamp range criteria
    And a paginated response is returned
    And response objects are ordered ascending by title
    And "accessible" key is part of the response
  ...
  Scenario: Only resources intersecting with a given Bounding Box are part of the response.
    ...
    When the request sends GET
    Then the response status is OK
    And 1 result(s) is(are) returned, when BoundingBox [1, 1, 2, 2] intersects with layers features
    And a paginated response is returned
    And response objects are ordered ascending by title
    And "accessible" key is part of the response
