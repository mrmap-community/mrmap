...
export const useDefaultWebSocket = (resourceTypes: string[] = []): DefaultWebSocket => {
  ...
  const { lastJsonMessage, lastMessage, sendMessage, sendJsonMessage, readyState, getWebSocket } = useWebSocket(defaultWsUrl, {
    shouldReconnect: () => {
      return true;
    },
  ...
  useEffect(() => {
    if (lastJsonMessage?.payload?.type){
      const inform = resourceTypes.length > 0 ? resourceTypes.find(resourceType => lastJsonMessage.payload.type.includes(resourceType)): true
      if (inform){
  ...
  return { lastResourceMessage, lastJsonMessage, lastMessage, sendMessage, sendJsonMessage, readyState, getWebSocket };
}
