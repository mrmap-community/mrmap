...
class GenericOwsServiceOperationFacade(View):
    ...
    def setup(self, request, *args, **kwargs):
        ...
        try:
            request.bbox = WebService.construct_polygon_from_bbox_query_param(
        ...
        if request.query_parameters.get("service").lower() == OGCServiceEnum.WMS.value:
            ...
            request.query_parameters.get("service").lower() == OGCServiceEnum.WFS.value
        ):
            service_cls = WebFeatureService

        self.service = service_cls.security.construct_service(
