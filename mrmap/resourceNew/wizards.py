...
def show_external_auth_form_condition(wizard):
    # try to get the cleaned data of step 1
    cleaned_data = wizard.get_cleaned_data_for_step(FIRST_STEP_ID) or {}
    # check if the field ``is_secured`` was checked.
    return cleaned_data.get('is_secured', True)


class NewResourceWizard(LoginRequiredMixin, PermissionRequiredMixin, SessionWizardView, ABC):
    success_url = reverse_lazy('resource:pending-tasks')
    permission_required = ["resourceNew.add_service"]
    raise_exception = True
    permission_denied_message = NO_PERMISSION
    condition_dict = {ACCESS_EDITOR_STEP_2_NAME: show_external_auth_form_condition}
