...
def register_ogc_service(job_pk, **kwargs):
    # if self.task:
    ...
    request = Request(
        method="GET", url=job.get_capabilities_url, auth=job.auth)
...
class RegisterOgcServiceJob(CommonInfo):
    ...
    collect_linked_metadata = models.BooleanField(
        default=True,
        verbose_name=_("collect metadata"),
        help_text=_(
            "auto start collecting task after successful registering "
            "which register all linked metadata records found in the "
            "GetCapabilities document."
        ),
    )
    registering_for_organization = models.ForeignKey(
        to=Organization,
        on_delete=models.DO_NOTHING,
        verbose_name=_("Registration for organization"),
        help_text=_(
            "Select for which organization you'd like to register the service."
        ),
    )
    auth = models.OneToOneField(
        to=ServiceAuthentication,
        on_delete=models.DO_NOTHING,
        null=True,
        blank=True,
        verbose_name=_("service authentication"),
        help_text=_(
            "Optional credentials to authenticate against the web service."),
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if adding:
            transaction.on_commit(
                lambda: register_ogc_service.delay(job_pk=self.pk, **kwargs)
            )
