...
class StructureRemovePublishRequestViewTestCase(TestCase):
    ...
    def test_invalid_remove_publish_request(self):
        perm = self.user.get_groups()[0].role.permission
        perm.can_remove_publisher = True
        perm.save()

        groups = create_guest_groups(how_much_groups=1)
        user = baker.make_recipe('tests.baker_recipes.structure_app.superadmin_user',
                                 groups=groups,
                                 username="Testuser2")
        org = create_non_autogenerated_orgas(user=user,
                                             how_much_orgas=1)
        user.organization = org[0]
        user.get_groups()[0].publish_for_organizations.add(org[0])

        post_params = {'is_accepted': True, }

        response = self.client.post(
            reverse('structure:remove-publisher',
                    args=(user.organization.id, user.get_groups()[0].id,)),
            post_params=post_params,
            HTTP_REFERER=HTTP_OR_SSL + HOST_NAME
        )
        ...
        self.assertEqual(response.status_code, 422)
        messages = [m.message for m in get_messages(response.wsgi_request)]
        self.assertIn(PUBLISH_PERMISSION_REMOVING_DENIED, messages)
