...
const getDescendants = (nodes: JsonApiPrimaryData[], currentNode: JsonApiPrimaryData) => {
  return nodes?.filter(
    (node: JsonApiPrimaryData) =>
      node?.attributes?.lft > currentNode?.attributes?.lft &&
      node?.attributes?.rght < currentNode?.attributes?.rght,
  );
};
...
const mapTreeData = (nodes: JsonApiPrimaryData[], currentNode: JsonApiPrimaryData): TreeNode => {
  ...
  if (children.length === 0) {
    ...
    children.forEach((child: JsonApiPrimaryData) => {
      childNodes.push(mapTreeData(nodes, child));
    });
    return {
      key: currentNode.id,
      title: currentNode.attributes.stringRepresentation,
...
const transformTreeData = (wms: JsonApiDocument): TreeNode[] => {
  const treeOrderedLayers = getIncludesByType(wms, 'Layer').sort(
    (a: JsonApiPrimaryData, b: JsonApiPrimaryData) => a.attributes.lft - b.attributes.lft,
  );
...
const transformFlatNodeList = (wms: JsonApiDocument): JsonApiPrimaryData[] => {
  return getIncludesByType(wms, 'Layer').sort(
    (a: JsonApiPrimaryData, b: JsonApiPrimaryData) => a.attributes.lft - b.attributes.lft,
  );
};
