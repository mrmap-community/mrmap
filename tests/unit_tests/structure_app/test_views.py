...
from structure.models import Organization, PendingTask, MrMapGroup, Permission, PublishRequest
...
class StructurePublishRequestViewTestCase(TestCase):
    ...
    def test_valid_publish_request(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse('structure:publish_request_overview')+f"?group={self.user.get_groups[0].id}&organization={self.user.organization.id}")
...
class StructureGroupInvitationRequestViewTestCase(TestCase):
    def setUp(self):
        ...
        self.groups = create_guest_groups(how_much_groups=9)
        self.user = create_superadminuser(groups=self.groups)
        self.test_user = create_testuser([self.groups[0]])
        self.orgas = create_non_autogenerated_orgas(user=self.user,
                                                    how_much_orgas=10)
