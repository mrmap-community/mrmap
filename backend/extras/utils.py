...
def build_queryset(base_qs, spec: dict):
    """
    Recursively build a queryset from a declarative spec.
    Resolves string model references lazily to avoid circular imports.
    """
    select_fields = spec.get("select")
    if select_fields:
        if isinstance(select_fields, (list, tuple)):
            base_qs = base_qs.select_related(*select_fields)
        else:
            base_qs = base_qs.select_related(select_fields)

    annotations = spec.get("annotate")
    if annotations:
        base_qs = base_qs.annotate(**annotations)

    prefetches = []

    for rel_name, rel_spec in spec.get("prefetch", {}).items():
        model_ref = rel_spec.get("model")
        if model_ref:
            if isinstance(model_ref, str):
                model_class = apps.get_model(model_ref)
            else:
                model_class = model_ref

            rel_qs = model_class.objects.all()
            rel_qs = build_queryset(rel_qs, rel_spec)
            prefetches.append(Prefetch(rel_name, queryset=rel_qs))
        else:
            prefetches.append(rel_name)

    if prefetches:
        base_qs = base_qs.prefetch_related(*prefetches)

    return base_qs


MERGEABLE_DICT_KEYS = {"annotate"}
RECURSIVE_KEYS = {"prefetch"}


def merge_spec_auto(base: dict, override: dict) -> dict:
    result = deepcopy(base)

    for key, value in override.items():
        if isinstance(value, dict) and isinstance(result.get(key), dict):
            result[key] = merge_spec_auto(result[key], value)
        else:
