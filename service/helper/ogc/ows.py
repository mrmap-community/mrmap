...
from structure.models import User, Contact, Organization
...
class ISOMetadata:
    ...
    def parse_xml(self):
        ...
        self.bounding_box["min_x"] = service_helper.try_get_text_from_xml_element(xml_obj, "//gmd:MD_Metadata/gmd:identificationInfo/{}/gmd:extent/gmd:EX_Extent/gmd:geographicElement/gmd:EX_GeographicBoundingBox/gmd:westBoundLongitude/gco:Decimal".format(xpath_type))
        self.bounding_box["min_y"] = service_helper.try_get_text_from_xml_element(xml_obj, "//gmd:MD_Metadata/gmd:identificationInfo/{}/gmd:extent/gmd:EX_Extent/gmd:geographicElement/gmd:EX_GeographicBoundingBox/gmd:southBoundLatitude/gco:Decimal".format(xpath_type))
        self.bounding_box["max_x"] = service_helper.try_get_text_from_xml_element(xml_obj, "//gmd:MD_Metadata/gmd:identificationInfo/{}/gmd:extent/gmd:EX_Extent/gmd:geographicElement/gmd:EX_GeographicBoundingBox/gmd:eastBoundLongitude/gco:Decimal".format(xpath_type))
        self.bounding_box["max_y"] = service_helper.try_get_text_from_xml_element(xml_obj, "//gmd:MD_Metadata/gmd:identificationInfo/{}/gmd:extent/gmd:EX_Extent/gmd:geographicElement/gmd:EX_GeographicBoundingBox/gmd:northBoundLatitude/gco:Decimal".format(xpath_type))
        ...
        if len(restriction_code_attr_val) >= 2:
            ...
            if restriction_code_attr_val[0] == 'license' and restriction_code_attr_val[1] == 'otherRestrictions':
                ...
                for constraint in other_constraints:
                    try:
                        constraint_json = json.loads(constraint)
                        self.license_source_note = constraint_json.get("quelle", None)
                        self.license_json = constraint.decode('string_escape')
                    except ValueError:
                        # no, this is not a json!
                        # handle it is a normal text
                        legal_constraints += constraint + ";"
