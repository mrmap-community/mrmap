...
class WmsHelper:
    ...
    def get_get_styles_url(self, layer: Layer):
        ...
        uri = self.service.operation_urls.filter(
            operation=OGCOperationEnum.GET_STYLES.value, method="Get"
        ).first()
    ...
    def get_get_legend_graphic_url(self):
        ...
        operation_url = self.service.operation_urls.filter(
            operation=OGCOperationEnum.GET_LEGEND_GRAPHIC.value, method="Get"
        ).first()
        ...
        service_format = (
            operation_url.mime_types.filter(mime_type__istartswith="image/")
            .exclude(mime_type__icontains="svg")
            .first()
        )
    ...
    def get_describe_layer_url(self):
        ...
        """
        uri = self.service.operation_urls.filter(
            operation=OGCOperationEnum.DESCRIBE_LAYER.value, method="Get"
        ).first()
        if uri is None:
            return
        uri = uri.url
    ...
    def get_get_feature_info_url(self):
        ...
        """
        uri = self.service.operation_urls.filter(
            operation=OGCOperationEnum.GET_FEATURE_INFO.value, method="Get"
        ).first()
    ...
    def get_get_map_url(self):
        ...
        """
        operation_url = self.service.operation_urls.filter(
            operation=OGCOperationEnum.GET_MAP.value, method="Get"
        ).first()
        if operation_url is None:
            return
